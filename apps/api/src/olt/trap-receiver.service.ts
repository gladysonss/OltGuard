...
import { INDICATION_OBJECT_OID, PARKS_TRAP_MAP, OltInternalEvent, ONU_DOWN_TRAP_OIDS } from './parks-trap-mapping';
...
export class TrapReceiverService implements OnModuleInit, OnModuleDestroy {
  ...
  private processNotification(notification: any) {
    ...
    const ingested = this.alarmIngest
      .ingest(parsed)
    ...
    // operacional ou nao. bLACKLISt e um estado direto (sem CLEAR nesta
    // MIB) - so seta. As demais traps de queda (down/sinal/energia, ver
    // ONU_DOWN_TRAP_OIDS) recalculam o status a partir de TODOS os alarmes
    // ACTIVE daquela posicao, nao so da trap que acabou de chegar - assim
    // uma ONU com lOSi e dGi simultaneos so volta a ACTIVE quando os dois
    // tiverem sido limpos, nao no CLEAR do primeiro. Por depender do Alarm
    // que acabou de ser gravado, so roda depois que `ingested` resolver
    // (encadeado com .then, nao em paralelo com o ingest acima).
    if (parsed.portNo !== undefined && parsed.logicalPortNo !== undefined) {
      const position = { slotNo: parsed.slotNo, portNo: parsed.portNo, logicalPortNo: parsed.logicalPortNo };
      if (parsed.event === OltInternalEvent.OnuBlacklisted) {
        void this.bootstrap
          .updateOnuStatusFromTrap(parsed.oltId, position, OnuStatus.DISABLE)
          .catch((err) => this.logger.error(`Falha ao atualizar status da ONU via trap: ${err.message}`));
      } else if (ONU_DOWN_TRAP_OIDS.includes(parsed.trapOid)) {
        void ingested.then(() =>
          this.bootstrap
            .reconcileOnuStatusFromAlarms(parsed.oltId, position, ONU_DOWN_TRAP_OIDS)
            .catch((err) => this.logger.error(`Falha ao reconciliar status da ONU via trap: ${err.message}`)),
        );
      }
    }
  }
